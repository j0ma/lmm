            ...
            for (name, field) in dataset.fields.items():
                if field is not None:
                    ...
                    if name == 'tgt':
                        ...
                        seq_lens = [x.size(1) for x in idx_batch]
                        max_seq_len = max(seq_lens)
                        idx_batch = [torch.cat([torch.Tensor([[2,2]+[1]*(x.size(0)-2)]).transpose(0,1).cuda(), x.cuda()], dim=1) for x in idx_batch]
                        idx_batch = [torch.cat([x.cuda(), torch.Tensor([[5,5,3,4]+[1]*(x.size(0)-4)]).transpose(0,1).cuda()], dim=1) for x in idx_batch]  
